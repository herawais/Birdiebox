...
class CalendarEvent(models.Model):
        ...
        if sf_access_token:
            ...
            if self.x_salesforce_id:
                ...
                if res.status_code == 204:
                    ...
                    self.env.user.company_id.export_event_lastmodifieddate = datetime.today()
            else:
    ...
    def exportCalendarEvent_to_sf(self):
        ...
        if self.duration:
            event_dict['DurationInMinutes'] = self.duration
        if self.start:
            event_dict['ActivityDateTime'] = str(self.start.date())
    ...
    def _scheduler_export_event_to_sf(self):
        ...
        if company_id:
            events = self.search([('write_date', '>', company_id.export_event_lastmodifieddate)])
            for event in events:
                try:
