...
class ResCompany(models.Model):
    ...
    opportunity_lastmodifieddate = fields.Datetime("Opportunity Last Modified Time :- ", default=_default_update_datetime)
    ...
    def sf_sendMessage(self, message):
        view_ref = self.env['ir.model.data'].get_object_reference('pragmatic_salesforce_connector', 'salseforce_message_wizard_form')
    ...
    def fetch_sf_cust_details(self, sf_id, is_account=False):
        ...
        if self.sf_access_token:
            ...
            if is_account:
                data = requests.request('GET', self.sf_url + '/services/data/v40.0/sobjects/account/' + str(sf_id), headers=headers)
            else:

                data = requests.request('GET', self.sf_url + '/services/data/v40.0/sobjects/Contact/' + str(sf_id), headers=headers)
            if data.status_code == 200:
                if data.text:
    ...
    def create_sf_contact(self, sf_contact_data):
        ...
        res_partner_srch = self.env['res.partner'].search([('x_salesforce_id', '=', str(sf_contact_data.get('Id'))), ('type', '=', 'contact')],
                                                          limit=1)
        if res_partner_srch:
        ...
        try:
            if self.sf_access_token:
                ...
                if product_partner != False and not type(product_partner) == type(dummy_dict) and is_account == False:
                    data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Contact where Id='{}'".format(
                        product_partner), headers=headers)
                elif is_account and product_partner != False and not type(product_partner) == type(dummy_dict):
                    data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Account where Id='{}'".format(
                        product_partner), headers=headers)
                elif is_account:
                    data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Account", headers=headers)
                else:
                    data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Contact", headers=headers)
                        ...
                        if ids_lst:
                            for id in ids_lst:
                                ...
                                if sf_cust_det:
                                    ''' Check if the Id from Salesforce is present in odoo or not if present
                                    then dont insert, This will avoid duplications'''
                                    res_partner = self.env['res.partner'].search([('x_salesforce_id', '=', str(sf_cust_det.get('Id')))])
                                    dict = {}
                                            ...
                                            if not sf_cust_det.get('AccountId'):
                                                if sf_cust_det.get('MailingAddress'):
                                                    ...
                                                    if sf_cust_det.get('MailingAddress').get('state'):
                                                        state_id = self.sf_attachCustomerState(sf_cust_det.get('MailingAddress').get('state'),
                ...
                if data:
                    ...
                    for rec in recs:
                        ...
                        if product_dict:
                            ...
                            self.product_lastmodifieddate = self.convert_sfdate_toodoo(product_read.get('LastModifiedDate'))
    ...
    def getSfQuoteData(self, quote_id):
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v40.0/sobjects/quote/{}".format(str(quote_id)), headers=headers)
            if data:
                if data.text:
                    parsed_data = json.loads(str(data.text))
                    if parsed_data:
    ...
    def getSfQuoteLineData(self, quote_id=None, is_so=False, so_id=None):
        if self.sf_access_token:
            headers = {}
            headers['Authorization'] = 'Bearer ' + str(self.sf_access_token)
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            if is_so and so_id:
                endpoint = "/services/data/v40.0/query/?q=select Id from orderitem where OrderId='{}'".format(str(so_id))
            else:
                endpoint = "/services/data/v40.0/query/?q=select Id from quotelineitem where QuoteId='{}'".format(str(quote_id))
                                    ...
                                    if data.text:
                                        ...
                                        if parsed_data.get('Id'):
                                            ...
                                            if product_exists:
                                                ...
                                                endpoint = "/services/data/v40.0/sobjects/product2/{}".format(str(parsed_data.get('Product2Id')))
                                                data = requests.request('GET', self.sf_url + endpoint, headers=headers)
                                                    ...
                                                    if data.text:
                                                        ...
                                                        if product_dict:
                                                            ...
                                                            if product_read.get('Family'):
                                                                ...
                                                                if categ_result:
                                                                    product_dict['categ_id'] = categ_result
                                                            result = self.create_sf_Product(product_dict, product_read.get('Id'))
                                                            if result:
    ...
    def getSfQuoteCustomer(self, opp_id):
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v40.0/sobjects/opportunity/{}".format(str(opp_id)), headers=headers)
            if data.status_code == 200:
                if data.text:
    ...
    def import_sf_quote(self):
        try:
            if self.sf_access_token:
                ...
                sf_modified_date = self.convert_odoodt_tosf(str(temp_odoo_date))
                data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from quote where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                                                sf_modified_date), headers=headers)
                if data.status_code == 200:
                    if data.text:
                        recs = json.loads(str(data.text))
                            ...
                            if recs.get('records'):
                                for r in recs.get('records'):
                                    ...
                                    if quote_data:
                                        ...
                                        ''' GET CUSTOMER '''
                                        if quote_data.get('OpportunityId') and not quote_data.get('ContactId'):
                                        ...
                                        if quote_data.get('ContactId'):
                                            ...
                                            ext_partner_id = self.env['res.partner'].search([('x_salesforce_id', '=', quote_data.get('ContactId'))])
                                            if ext_partner_id:
                                                quote_dict['partner_id'] = ext_partner_id.id
                                            else:
                                                partner_id = self.import_sf_cust(product_partner=quote_data.get('ContactId'))
                                                if partner_id:
                                                    quote_dict['partner_id'] = partner_id[0]

                                        if quote_data.get('AccountId'):
                                            ext_partner_id = self.env['res.partner'].search([('x_salesforce_id', '=', quote_data.get('AccountId'))])
                                            if ext_partner_id:
                                                quote_dict['partner_id'] = ext_partner_id.id
                                            else:
                                                # Check if contact Id exists or not, If yes then create that custom First
                                                partner_id = self.import_sf_cust(product_partner=str(quote_data.get('AccountId')), is_account=True)
                                                if partner_id:
                                                    quote_dict['partner_id'] = partner_id[0]
                                        if quote_dict:
                                            # Create Quotation
                                            # Check if quotation dosent exists
                                            quote_exists = self.env['sale.order'].search([('x_salesforce_id', '=', quote_data.get('Id'))])
                                            if not quote_exists:
                                                quote_create = self.env['sale.order'].sudo().create(quote_dict)
                                                if quote_create:
                                                    quote_create.sudo().write({'x_salesforce_id': quote_data.get('Id')})
                                                    self.quote_lastmodifieddate = self.convert_sfdate_toodoo(quote_data.get('LastModifiedDate'))
                                                    ...
                                                    if quote_line_data:
                                                        for soline in quote_line_data:
                                                            ...
                                                            if not quote_line_exists:
                                                                ...
                                                                soline.pop('Id')
                                                                quote_line_create = self.env['sale.order.line'].create(soline)
                                                                if quote_line_create:
                                                                    quote_line_create.sudo().write({'x_salesforce_id': soline.get('Id')})
                                            else:
                                                quote_exists.sudo().write(quote_dict)
                                                self.quote_lastmodifieddate = self.convert_sfdate_toodoo(quote_data.get('LastModifiedDate'))
                                                # Attach Order id to sale order line
                                                order_line = {}
                                                if quote_line_data:
                                                    for soline in quote_line_data:
                                                        ...
                                                        if not quote_line_exists:
                                                            soline.update({'x_salesforce_id': soline.get('Id')})
                                                            soline.pop('Id')
                                                            quote_line_create = self.env['sale.order.line'].sudo().create(soline)
                                                            if quote_line_create:
                                                                quote_line_create.sudo().write({'x_salesforce_id': soline.get('Id')})
                                                        else:
                                                            quote_line_exists.sudo().write(soline)
        except Exception as e:
            raise UserError(_("Oops Some error Occured" + str(e)))
    ...
    def get_sf_so_cust(self, so_id):
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v39.0/sobjects/order/{}".format(str(so_id)), headers=headers)
            if data.status_code == 200:
                if data.text:
                    recs = json.loads(str(data.text))
                    if recs:
                        ...
                        if cust_exists:
                            ...
                            if recs.get('AccountId'):
                                ...
                                ext_partner_id = None
                                # Check if contact Id exists or not, If yes then create that custom First
                                ext_partner_id = self.env['res.partner'].search([('x_salesforce_id', '=', recs.get('AccountId'))])
                                if not ext_partner_id:
                                    partner_id = self.import_sf_cust(product_partner=str(recs.get('AccountId')), is_account=True)
                                if partner_id:
                                    return partner_id[0]
                                elif ext_partner_id:
    ...
    def get_sf_so_data(self, sf_id):
        ...
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v40.0/sobjects/order/{}".format(str(sf_id)), headers=headers)
            if data.status_code == 200:
                if data.text:
                    recs = json.loads(str(data.text))
                    if recs:
                        ...
                        if recs.get('Description'):
                            sf_order_dict['note'] = 'Order No:' + recs.get('OrderNumber') + ' ' + recs.get('Description')
    ...
    def import_sf_so(self):
        # try:
            if self.sf_access_token:
                ...
                sf_modified_date = self.convert_odoodt_tosf(str(temp_odoo_date))
                data = requests.request('GET', self.sf_url + "/services/data/v39.0/query/?q=select Id from order where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                                                sf_modified_date), headers=headers)
                if data.status_code == 200:
                    if data.text:
                        parsed_data = json.loads(str(data.text))
                        if parsed_data and parsed_data.get('records'):
                            for rec in parsed_data.get('records'):
                                ...
                                if so_data:
                                    ...
                                    # Check if quotation dosent exists
                                    so_exists = self.env['sale.order'].search([('x_salesforce_id', '=', rec.get('Id'))],limit=1)
                                    if not so_exists:
                                        so_create = self.env['sale.order'].sudo().create(so_data)
                                        if rec.get('LastModifiedDate'):
                                            self.order_lastmodifieddate = self.convert_sfdate_toodoo(rec.get('LastModifiedDate'))
                                        if so_create:
                                            so_create.sudo().write({'x_salesforce_id': rec.get('Id')})
                                            # Attach Order id to sale order line
                                            order_line = {}
                                            ...
                                            if so_line_data:
                                                for soline in so_line_data:
                                                    ...
                                                    # Check if quotation line dosent exists
                                                    so_line_exists = self.env['sale.order.line'].search([('x_salesforce_id', '=', soline.get('Id'))],limit=1)
                                                    if not so_line_exists:
                                                        soline.pop('Id')
                                                        so_line_create = self.env['sale.order.line'].sudo().create(soline)
                                                        if so_line_create:
                                                            so_line_create.sudo().write({'x_salesforce_id': soline.get('Id')})
                                    else:
                                        so_exists.sudo().write(so_data)
                                        if rec.get('LastModifiedDate'):
                                            self.order_lastmodifieddate = self.convert_sfdate_toodoo(rec.get('LastModifiedDate'))
    ...
    def create_sf_company(self, sf_account_data):
        ...
        res_partner_srch = self.env['res.partner'].search([('x_salesforce_id', '=', str(sf_account_data.get('Id'))), ('type', '=', 'contact')],
                                                          limit=1)
        sf_modified_dt = self.convert_sfdate_toodoo(sf_account_data.get('LastModifiedDate'))
        sf_account_data = self.get_sf_customer_required_details(sf_account_data.get('Id'), is_account=True)  # get all data of an account from salesforce
    ...
    def fetch_sf_lead_details(self, rec):
        ...
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v40.0/sobjects/Lead/" + str(rec), headers=headers)
            if data.status_code == 200:
                if data.text:
    ...
    def import_sf_lead(self):
        try:
            if self.sf_access_token:
                ...
                sf_modified_date = self.convert_odoodt_tosf(str(temp_odoo_date))
                data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Lead where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                                                sf_modified_date), headers=headers)
                if data:
                    recs = []
                    parsed_data = json.loads(str(data.text))
    ...
    def fetch_sf_opportunity_details(self, rec):
        ...
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v40.0/sobjects/Opportunity/" + str(rec), headers=headers)
            if data.status_code == 200:
                if data.text:
    ...
    def import_sf_opportunity(self):
        try:
            if self.sf_access_token:
                ...
                sf_modified_date = self.convert_odoodt_tosf(str(temp_odoo_date))
                data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Opportunity where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                                                sf_modified_date), headers=headers)
                if data:
                    recs = []
                    parsed_data = json.loads(str(data.text))
                    ...
                    for rec in recs:
                        ...
                        if opportunity_dict:
                            ...
                            self.opportunity_lastmodifieddate = self.convert_sfdate_toodoo(opportunity_read.get('LastModifiedDate'))
        except Exception as e:
            raise UserError(_("Oops Some error Occured" + str(e)))
    ...
    def fetch_sf_contract_details(self, rec):
        ...
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v40.0/sobjects/Contract/" + str(rec), headers=headers)
            if data.status_code == 200:
                if data.text:
    ...
    def create_sf_Contract(self,contract_dict, sf_id):
        ...
        if not contract_exists:
            ...
            contract_exists.sudo().write(contract_dict)
    ...
    def import_sf_contract(self):
        try:
            if self.sf_access_token:
                ...
                sf_modified_date = self.convert_odoodt_tosf(str(temp_odoo_date))
                data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Contract where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                                                sf_modified_date), headers=headers)
                if data:
                    recs = []
                    parsed_data = json.loads(str(data.text))
                    ...
                    for rec in recs:
                        ...
                        if contract_dict:
                            ...
                            self.contract_lastmodifieddate = self.convert_sfdate_toodoo(contract_read.get('LastModifiedDate'))
        except Exception as e:
            raise UserError(_("Oops Some error Occured" + str(e)))
    ...
    def create_sf_Event(self,event_dict, sf_id):
        ...
        headers['accept'] = 'application/json'
        headers['Content-Type'] = 'text/plain'
        event_exists = event_obj.search([('x_salesforce_id', '=', sf_id)])
        if not event_exists:
            ...
            event_exists.sudo().write(event_dict)
    ...
    def fetch_sf_activity_details(self, rec):
        ...
        if self.sf_access_token:
            ...
            headers['accept'] = 'application/json'
            headers['Content-Type'] = 'text/plain'
            data = requests.request('GET', self.sf_url + "/services/data/v40.0/sobjects/Task/" + str(rec), headers=headers)
            if data.status_code == 200:
                if data.text:
    ...
    def import_sf_activity(self):
        try:
            if self.sf_access_token:
                ...
                sf_modified_date = self.convert_odoodt_tosf(str(temp_odoo_date))
                data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Task where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                                                sf_modified_date), headers=headers)
                if data:
                    recs = []
                    parsed_data = json.loads(str(data.text))
                    ...
                    for rec in recs:
                        ...
                        if activity_dict:
                            ...
                            self.task_lastmodifieddate = self.convert_sfdate_toodoo(activity_read.get('LastModifiedDate'))
        except Exception as e:
            raise UserError(_("Oops Some error Occured" + str(e)))
